...
from paip.pipelines.report.generate_reports import GenerateReports
import paip.pipelines.report.generate_reports
...
def test_run(sample_task_factory, mock_rename, monkeypatch):
    ...
    monkeypatch.setattr(paip.pipelines.report.generate_reports,
                        'ReportsPipeline', ReportsPipeline)

    open_ = mock_open()
    with patch('paip.pipelines.report.generate_reports.open', open_):
        task.run()
