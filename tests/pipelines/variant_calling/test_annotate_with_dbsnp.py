...
from paip.pipelines.variant_calling import AnnotateWithDbSNP, MergeVCFs, JointGenotyping
...
def test_run(variant_sites_task, test_cohort_path, monkeypatch):
    ...
    # be a better way:
    with patch('paip.pipelines.variant_calling.annotate_with_dbsnp.open', open_):
        task.run()
