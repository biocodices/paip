...
def test_run(cohort_task_factory):
    ...
    assert re.search(r'--warning_file .+/Cohort1/Cohort1..+.warning.txt', command)
    assert re.search(r'--format vcf ', command)
    assert re.search(r'--everything ', command)
    assert re.search(r'--fork 1 ', command)
    assert re.search(r'--vcf ', command)
    assert re.search(r'--compress_output bgzip ', command)
    assert re.search(r'--offline ', command)
    assert re.search(r'-i .+/Cohort1/Cohort1..+.vcf', command)
    assert re.search(r'-o .+/Cohort1/Cohort1..+.vep.vcf.gz-luigi-tmp-.+', command)
